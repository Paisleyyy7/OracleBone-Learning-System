...
class Data:
    ...
    def get_loaders(self, parse_patches=True, test=False):
        ...
        # 训练数据加载器
        if parse_patches:
            # 如果启用分布式训练，使用 `DistributedSampler`
            ...
            # 如果不使用分布式训练，直接使用常规的 DataLoader
            train_loader = torch.utils.data.DataLoader(train_dataset,
                                                       batch_size=self.config.training.batch_size,
                                                       shuffle=True,  # 如果不使用分布式，可以开启 shuffle
                                                       num_workers=self.config.data.num_workers,
                                                       prefetch_factor=2,
                                                       pin_memory=True)
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def get_images(self, index):
        ...
        input_img = PIL.Image.open(os.path.join(self.dir,input_name)).convert(
            'RGB') if self.dir else PIL.Image.open(input_name)
        gt_img = PIL.Image.open(os.path.join(self.dir,gt_name)).convert(
